...
> **Why IDs change:** Each `revise` call creates a brand-new resource ID in Launch. After a `sync + publish`, the IDs in your local files are stale. **The CI/CD workflow handles this automatically** by performing a `pull` after every successful sync and committing the new IDs back to your branch. You only need to run `pull` locally if you want to see the new IDs immediately without waiting for the CI commit.
...
3. **Push to GitHub** — the CI/CD workflow will automatically:
   - Run `sync` to push to Launch (and auto-publish if in environment mode)
   - Run `pull` to fetch the new resource IDs generated by Launch
   - **Commit the updated IDs back to your branch** (with `[skip ci]`)
4. **Pull from GitHub** — run `git pull` locally after the CI finishes to get the updated resource IDs in your local environment. This keeps your local copy aligned with Adobe Launch without having to run `pull` manually.
...
### Automatic alignment of resource IDs

Adobe Launch generates new IDs every time a resource is updated and revised. This means that after a successful `sync`, the IDs in Git are technically obsolete.

To solve this, **the workflow automatically performs a `pull` after every successful `sync` and commits the results back to your branch.**

- The commit message will be `chore: update resource IDs from Launch [skip ci]`.
- The `[skip ci]` tag prevents the workflow from triggering itself in an infinite loop.
- **You should run `git pull` locally** after the CI job finishes to stay in sync with these automatic updates.
